...
""" PyTorch Unpadded & Fused Phi model. Compatible with HF. """
...
#not use
# class UnpaddedPhiRMSNorm(nn.Module):
#     def __init__(self, hidden_size, eps):
#         """
#         UnpaddedPhiRMSNorm is equivalent to T5LayerNorm
#         """
#         super().__init__()
...
class UnpaddedPhiRotaryEmbedding(torch.nn.Module):
...
# OK except flash_attn_varlen_func
...
# OK except UnpaddedPhiMLP and (UnpaddedPhiAttention or UnpaddedPhiAttention2)
class UnpaddedPhiDecoderLayer(nn.Module):
    def __init__(self, config: PhiConfig):
        super().__init__()

        self.hidden_size = config.hidden_size
        self.self_attn = UnpaddedPhiAttention(config=config)
        self.mlp = UnpaddedPhiMLP(
            hidden_size=self.hidden_size,
            intermediate_size=config.intermediate_size,
            hidden_act=config.hidden_act,
        )
        # self.input_layernorm = UnpaddedPhiRMSNorm(config.hidden_size, eps=config.layer_norm_eps)
        # self.post_attention_layernorm = UnpaddedPhiRMSNorm(config.hidden_size, eps=config.layer_norm_eps)
...
# OK except UnpaddedPhiDecoderLayer
class UnpaddedPhiModel(UnpaddedPhiPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`UnpaddedPhiDecoderLayer`]
    ...
    def __init__(self, config: PhiConfig):
        ...
        self.rotary_emb   = UnpaddedPhiRotaryEmbedding(config.hidden_size // config.num_attention_heads,
                                                         max_position_embeddings=config.max_position_embeddings,
                                                         base=config.rope_theta)

        self.layers = nn.ModuleList([UnpaddedPhiDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        # self.norm = UnpaddedPhiRMSNorm(config.hidden_size, eps=config.layer_norm_eps)
